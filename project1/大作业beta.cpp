...
//		int r1=msg.y/PIC_SIZE+1;
//    	int c1=msg.x/PIC_SIZE+1;
...
//		if(r!=r1||c!=c1){
//			if(click[r][c]<=0){
//				putimage((c-1)*PIC_SIZE,(r-1)*PIC_SIZE,&img[17-click[r][c]]);
//			}
//			if(click[r1][c1]<=0){
//				putimage((c1-1)*PIC_SIZE,(r1-1)*PIC_SIZE,&img[10-click[r1][c1]]);
//			}
//			r1=r;
//			c1=c;
//		}
...
void drawMineCount(void){
    ...
    fillrectangle(COL*PIC_SIZE-2.5*PIC_SIZE-11,ROW*PIC_SIZE+4,COL*PIC_SIZE-PIC_SIZE-1,ROW*PIC_SIZE+36);
    sprintf(buffer,"%d",MINE_NUM-flag);
    settextcolor(YELLOW);
    settextstyle(30,15,"Consolas");
    outtextxy(COL*PIC_SIZE-2.5*PIC_SIZE-10,ROW*PIC_SIZE+5,buffer);
}
...
void draw(void){
	...
	fillrectangle(PIC_SIZE+1,ROW*PIC_SIZE+4,PIC_SIZE+115,ROW*PIC_SIZE+36);
	putimage(0,ROW*PIC_SIZE,&img[13]);
	putimage(COL*PIC_SIZE-PIC_SIZE,ROW*PIC_SIZE,&img[14]);
	settextcolor(BLUE);
	setbkcolor(RGB(189,189,189));
    settextstyle(26,13,"Consolas");
    outtextxy(PIC_SIZE+5,ROW*PIC_SIZE+7,"00:00:00");
}
void end(void){
	playmusic("bomb.wav");
	int i,j;
...
int mouse(void){
  ...
  if(MouseHit()){
    ...
    int r=msg.y/PIC_SIZE+1;
    int c=msg.x/PIC_SIZE+1;
    if(r==ROW+1&&(c==COL/2||c==COL/2+1)&&(msg.uMsg==WM_LBUTTONDOWN||msg.uMsg==WM_RBUTTONDOWN)){
...
void game(void){
  ...
  while(1){
    ...
    static int lastB = 0;
      ...
      else if(GetAsyncKeyState(VK_RBUTTON)&0x8000&&(clock()-lastB)>100){
      	rclick(r,c);
      	lastB = clock();
      }
    }
...
void select(void){
	...
	if(MouseHit()){
		...
		if(msg.y<100) continue;
			...
			while(ROW>24||COL>30||MINE_NUM>=ROW*COL*0.9||ROW<=0||COL<=0||MINE_NUM<=0){
...
void start(void){
	select();
	initgraph(PIC_SIZE*COL,PIC_SIZE*(ROW+1),SHOWCONSOLE);
