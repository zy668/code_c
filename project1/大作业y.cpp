...
void select(void){
	...
	initgraph(500, 500);
	HWND hwnd = GetHWnd();
	MoveWindow(hwnd, 350, 180,500,530,TRUE);
	...
	if(MouseHit()){
		...
		if(msg.y<100) continue;
			...
			while(ROW>24||COL>30||10*MINE_NUM>=ROW*COL*9||ROW<=0||COL<=0||MINE_NUM<10){
...
void start(void){
	...
	HWND hwnd=GetHWnd();
	MoveWindow(hwnd,350,180,PIC_SIZE*COL+6,PIC_SIZE*(ROW+1)+29,TRUE);
