...
void timer(void) {
	...
	wchar_t pass[50];
	swprintf(pass, 50, L"%02d:%02d:%02d", hours, minutes, seconds);
	settextcolor(BLUE);
	settextstyle(26, 13, L"Consolas");
...
void drawMineCount(void) {
	...
	swprintf(buffer, 41, L"%d", MINE_NUM - flag);
	settextcolor(YELLOW);
	settextstyle(30, 15, L"Consolas");
	outtextxy(COL * PIC_SIZE - 2.5 * PIC_SIZE - 10, ROW * PIC_SIZE + PIC_SIZE / 8, buffer);
}

void playmusic(const wchar_t* music) {
	std::wstring str;
	str = L"close " + std::wstring(music);
	mciSendStringW(str.c_str(), 0, 0, 0);

	str = L"open " + std::wstring(music);
	mciSendStringW(str.c_str(), 0, 0, 0);

	str = L"play " + std::wstring(music);
	mciSendStringW(str.c_str(), 0, 0, 0);
}
void refresh(void) {
	int i, j;
...
void draw(void) {
	...
	setbkcolor(RGB(189, 189, 189));
	settextstyle(26, 13, L"Consolas");
	outtextxy(PIC_SIZE + 5, ROW * PIC_SIZE + PIC_SIZE / 5, L"00:00:00");
}
void end(void) {
	timer();
	playmusic(L"bomb.wav");
	int i, j;
	for (i = 1; i <= ROW; i++) {
		for (j = 1; j <= COL; j++) {
			if (map[i][j] < 0) {
...
void game(void) {
	...
	while (1) {
		...
		if (mouse()) {
			...
			isok = MessageBox(GetHWnd(), L"Bad luck,You lose!\nDo you want to play again?", L"notice", MB_OKCANCEL);
			playmusic(L"lose.mp3");
			break;
		}
		else if (Count == ROW * COL - MINE_NUM) {
			playmusic(L"win.wav");
			wchar_t buffer[80];
			swprintf(buffer, 80, L"You win!\ntotal time: %ls\nDo you want to play again?", pass);
			isok = MessageBox(GetHWnd(), buffer, L"notice", MB_OKCANCEL);
			break;
		}
	}
}
...
//	MoveWindow(hwnd, 350, 180, 500, 540, TRUE);
	...
	outtextxy(100, 50, L"请选择难度：");
	rectangle(40, 100, 350, 200);
	outtextxy(100, 150, L"初级9*9，雷数10");
	rectangle(40, 200, 350, 300);
	outtextxy(100, 250, L"中级16*16，雷数40");
	rectangle(40, 300, 350, 400);
	outtextxy(100, 350, L"高级16*30，雷数99");
	rectangle(40, 400, 350, 520);
	outtextxy(130, 420, L"自定义");
	outtextxy(50, 455, L"雷区大小不能超过24x30，最小为9x9");
	outtextxy(50, 485, L"雷数最少10个，最多不能超过雷区的90%");
	Sleep(10);
	while (1) {
		if (MouseHit()) {
			MOUSEMSG msg = GetMouseMsg();
			if (msg.uMsg == WM_LBUTTONDOWN) {
				if (msg.x > 40 && msg.x < 350) {
					if (msg.y < 100) continue;
						...
						wchar_t s[20];
						bool t = InputBox(s, 20, L"请输入行数和列数和雷数，用空格分开", L"自定义", NULL, 0, 0, 0);
						if (t) swscanf(s, L"%d %d %d", &ROW, &COL, &MINE_NUM);
						while (t && (ROW > 24 || COL > 30 || 10 * MINE_NUM >= ROW * COL * 9 || ROW <= 0 || COL <= 0 || MINE_NUM < 10)) {
							t = InputBox(s, 20, L"invalid,请重新输入行数和列数和雷数，用空格分开", L"自定义", NULL, 0, 0, 0);
							swscanf(s, L"%d %d %d", &ROW, &COL, &MINE_NUM);
						}
						if (t) break;
					}
				}
