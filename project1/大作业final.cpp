...
#define PIC_SIZE 40
using namespace std;
char Rank[4][40]={"a999999999:99:99","a999999999:99:99","a999999999:99:99","a999999999:99:99"};
IMAGE img[20];
int map[26][32];
int click[26][32]={0};
int flag=0;
...
void game(void){
  ...
  while(1){
    ...
    if(mouse()){
      ...
      playmusic("win.wav");
      char buffer[100];
      if(strcmp(Rank[difficulty-1],pass)>0){
      	strcpy(Rank[difficulty-1],pass);
		sprintf(buffer,"New Record!\nBest time:%s\ntotal time: %s\nDo you want to play again?",Rank[difficulty-1],pass);
	  }
      else{
